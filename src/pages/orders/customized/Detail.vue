			...
			<InfoRow title="訂購人">
				<div class="q-my-md">
					...
					<div v-if="member.selected.length > 0" class="row q-gutter-md q-mt-none">
						<q-input v-for="column in customizedMemberColumns" :key="column.name" v-model="member.selected[0][column.field]" :label="column.label" class="col" dense outlined readonly />
					</div>
				</div>
			</InfoRow>
			<InfoRow title="訂單資訊">
...
onMounted(async () => {
	...
	if (!isNaN(orderNumber)) {
		...
		if (order) {
			...
			data.content = JSON.parse(order.content);
